"use client";
import { Button } from "@/components/ui/button";
import { useRouter } from "next/navigation";
import Image from "next/image";

export const SuccessBookView = () => {
  const router = useRouter();
  return (
    <div className="relative flex flex-col w-full gap-8 font-serif mx-auto pt-8 min-h-dvh text-brand-500">
      <div className="flex flex-col items-center gap-4 text-center mt-12 /">
        <Image src={"/assets/view/success.png"} alt="success" width={120} height={120} />
        <p className="font-extrabold text-xl">Payment Success</p>
        <p className="font-normal">You’re all set! Your spot at Sehela Space is confirmed.</p>
        <Button className="min-h-12 text-sm font-extrabold" onClick={() => router.push("/profile/my-sessions")}>
          View My Class
        </Button>
      </div>
    </div>
  );
};
