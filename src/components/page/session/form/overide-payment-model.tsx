...
export const OveridePaymentModelForm = ({ prefix, label, payment_model }: IProps) => {
  const methods = useFormContext();

  const { control, watch, setValue } = methods;
  const regular = watch("isOveride");
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-row items-center gap-1 justify-between">
          <p className="text-md font-semibold">Instructor Payment Model</p>
          <FormField
            control={control}
            name={"isOveride"}
            render={({ field }) => (
              <FormItem className="flex flex-row w-[250px] justify-between items-center">
                <FormLabel className="text-[#171717] font-[600] text-[16px]">{label}</FormLabel>
